...
function getYoutubeEmbedUrl(nocookie: boolean, isPlaylist = false) {
  if (isPlaylist) {
...
export function getEmbedUrlFromYoutubeUrl(options: YoutubeEmbedOptions) {
  const {
    url,
    allowFullscreen,
    autoplay,
  ...
  let outputUrl = `${getYoutubeEmbedUrl(
    nocookie,
    matches[1] === "list",
  )}${matches[2]}`;
  const params: string[] = [];
