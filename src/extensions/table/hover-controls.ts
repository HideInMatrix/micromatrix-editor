...
export type TableHoverAxis = 'row' | 'column'

export type TableHoverControl = {
  axis: TableHoverAxis
...
const isPointerWithinControl = (
  event: MouseEvent,
  control: TableHoverControl | null,
  container: HTMLElement,
) => {
  if (!control) {
    return false
...
const resolveCellSelectionPos = (editor: Editor, cell: HTMLTableCellElement) => {
...
export const useTableHoverControls = (
  editor: Ref<Editor | null | undefined>,
  containerRef: Ref<HTMLElement | null>,
) => {
  const activeControl = ref<TableHoverControl | null>(null)
  ...
  const refreshActiveControl = () => {
    ...
    activeControl.value = createTableHoverControl(
      control.axis,
      control.table,
      container,
    )
  ...
  const addColumnAtEnd = () => {
    ...
    const success = (currentEditor.chain().focus().setTextSelection(pos) as any)
      .addColumnAfter()
      .run()
  ...
  watch(
    [editor, containerRef],
    ([currentEditor, container], _oldValue, onCleanup) => {
      activeControl.value = null
