  ...
  >
    ...
    >
      ...
      <footer class="umo-footer">
        <AsyncStatusbar v-if="statusbarLoaded" />
      </footer>
    </div>
  </t-config-provider>
</template>
...
const AsyncToolbar = defineAsyncComponent(() => import('./toolbar/index.vue'))
const AsyncStatusbar = defineAsyncComponent(
  () => import('./statusbar/index.vue'),
)
const loadDomToImage = async () => {
  if (!domToImageLoader) {
    domToImageLoader = import('dom-to-image-more')
  }
  return await domToImageLoader
}

let docxSerializerLoader = null
const loadDocxSerializer = async () => {
  if (!docxSerializerLoader) {
    docxSerializerLoader = import('@/extensions/docx-export/serializer')
  }
  return await docxSerializerLoader
}
...
const statusbarLoaded = ref(false)
const aiChat = ref({
  visible: false,
  scope: 'auto',
  focusToken: 0,
...
provide('aiChat', aiChat)
provide('openAiChat', (payload = {}) => {
  aiChat.value = {
    ...aiChat.value,
    visible: true,
    scope: payload.scope || 'auto',
    selectionAnchor: payload.selectionAnchor || null,
    focusToken: (aiChat.value.focusToken || 0) + 1,
  }
})
provide('closeAiChat', () => {
  aiChat.value = {
...
// Lifecycle Hooks
let statusbarLoadFrame = 0
onMounted(() => {
  const theme = useStorage('umo-editor:theme', options.value.theme)
  const skin = useStorage('umo-editor:skin', options.value.skin)
  setTheme(theme.value)
  setSkin(skin.value)
  statusbarLoadFrame = requestAnimationFrame(() => {
    toolbarLoaded.value = true
    statusbarLoaded.value = true
    statusbarLoadFrame = 0
  })
})
onBeforeUnmount(() => {
  if (statusbarLoadFrame) {
    cancelAnimationFrame(statusbarLoadFrame)
  }
...
const getImage = async (format = 'blob') => {
  ...
  try {
    ...
    const { toBlob, toJpeg, toPng } = domToImage.default || domToImage
...
const exportDocx = async (customOptions = {}) => {
  ...
  const { createDocxBlob } = await loadDocxSerializer()
