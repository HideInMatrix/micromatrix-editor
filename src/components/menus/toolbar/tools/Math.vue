<template>
  ...
  >
    ...
    >
      ...
      <div ref="containerRef" class="mxm-math-container">
        <div class="mxm-math-input">
          <TTextarea
            ...
            @change="previewMath"
          />
          <div class="mxm-math-input-preview mxm-scrollbar">
            <div v-if="latexValue !== ''" class="mxm-math-input-render"></div>
            <div v-else class="mxm-math-input-preview-empty">
              {{ t('tools.math.preview') }}
            </div>
          </div>
        </div>
        <div class="mxm-math-select">
          <div
            v-for="(item, index) in templates"
            v-show="latexLoaded"
            :key="index"
            class="mxm-math-select-item"
            @click="selectMath(item)"
          >
            {{ item }}
          </div>
          <div v-if="!latexLoaded" class="mxm-math-select-loading">
            {{ t('tools.math.template') }}
          </div>
        </div>
      </div>
      <template #footer>
        <div class="mxm-math-footer">
          <TButton
            theme="default"
            variant="base"
          ...
          <TDropdown
            ...
            @click="insertMath"
          >
            <TButton :disabled="latexValue === ''">
              {{ t('tools.math.confirm') }}
            </TButton>
          </TDropdown>
          <TButton v-else :disabled="latexValue === ''" @click="updateMath">
            {{ t('tools.math.update') }}
          </TButton>
        </div>
      </template>
...
const renderMath = async (type) => {
  ...
  if (type === 'templates') {
    containerRef.querySelectorAll('.mxm-math-select-item').forEach((el) => {
      render(el.textContent, el)
    })
  }
  if (type === 'preview' && latexValue !== '') {
    await nextTick()
    const el = containerRef.querySelector('.mxm-math-input-render')
